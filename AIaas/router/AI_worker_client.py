...
class AIWorkerClient:
    def __init__(self, load_balancer):
        ...
        """初始化时模型信息位空，需要特殊处理"""
        worker_id_to_worker_map = self.load_balancer.get_all_workers()
        ...
        """此处注意：worker_id_to_worker_map非空时返回的里层字典需要用json.loads解析出worker信息"""
        if worker_id_to_worker_map:
            for worker_id, worker in worker_id_to_worker_map.iteritems():   
                self.worker_to_AI_client_map[worker_id] = self.make_clients(json.loads(worker))
