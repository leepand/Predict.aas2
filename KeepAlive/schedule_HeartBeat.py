...
class BaseScheduler:
    ...
    def main_func(self, task):
        ...
        worklist_db=redis_hget(conn,key)
        ...
        for model_id,worklist in worklist_db.items():
            ...
            for worker_id in json.loads(worklist):
                per_worker=json.loads(redis_hget(conn,worker_info)[worker_id])
        ...
        new_list_dict[model_id]=json.dumps(new_list)
        
        dict_to_redis_hset(conn, key, new_list_dict)    
